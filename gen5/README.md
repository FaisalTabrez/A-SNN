...
trains early multi-timescale LIF state to predict later encoder features. Its
paired-future candidate is tested against no-predictive, shuffled-target, and
analog controls in one resumable SSC runner:

```powershell
python gen5/examples/gen7_predictive_state.py --list-arms
```

Gen-7 returned `stop`. Paired prediction learned strong future alignment and
slightly exceeded TCN mean accuracy, but state shuffling improved accuracy and
...
29. Executable axonal-delay ablation
   - makes `DynamicSparseLinear.delay_steps` causal through history buckets
   - preserves sensor timing while delaying only recurrent edges
