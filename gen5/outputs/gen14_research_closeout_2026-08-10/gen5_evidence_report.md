...
| ---: | --- | --- | --- |
| 1 | gen14_terminal_evidence_freeze | Package the solvable embodied control and failed reward-specific eligibility gate. | A reproducible 16-source ledger whose claims match the Gen-14 stop decision. |
| 2 | publication_package | Report the supported mechanism chain through Gen-14 without architecture-superiority claims. | Exact protocols, seeds, checkpoints, causal controls, and negative gates are publication-ready. |
| 3 | local_learning_program_closed | Keep STW/LTW, replay, structural plasticity, and local-rule tuning closed after Gen-13 and Gen-14 terminal failures. | No continuous-learning claim is made from mechanisms that failed identity-specific causal controls. |
| 4 | theory_and_baseline_reset | Freeze empirical mechanism additions and require a matched conventional reward-learning baseline plus a reset evaluation protocol before another local-learning hypothesis. | A new design explains how credit, non-stationarity, and agent identity will be separated before implementation. |
