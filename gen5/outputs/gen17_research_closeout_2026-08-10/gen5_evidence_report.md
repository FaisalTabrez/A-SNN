...
| ---: | --- | --- | --- |
| 1 | gen17_sparse_translation_closeout | Package the failed Bernoulli translation and the non-replicating analog reference. | A reproducible 19-source ledger separates operational spikes and gradients from failed behavioral credit. |
| 2 | publication_package | Report the supported mechanism chain through Gen-17 without architecture-superiority claims. | Exact protocols, seeds, checkpoints, causal controls, and negative gates are publication-ready. |
| 3 | gen18_held_out_local_credit_replication | Replicate the analog local-credit gain and reward identity on ten untouched seeds. | At least 7/10 seeds qualify and lower 95% confidence bounds for gain and reward identity are positive. |
| 4 | complex_plasticity_remains_gated | Keep new spike encoders, STW/LTW, replay, and structural plasticity closed until analog credit replicates. | No new mechanism is added before the Gen-18 held-out gate passes. |
