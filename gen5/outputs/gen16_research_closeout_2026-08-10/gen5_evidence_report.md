...
| ---: | --- | --- | --- |
| 1 | gen16_local_credit_closeout | Package exact local/autograd credit equivalence and its small behavioral effect. | A reproducible 18-source ledger that separates analog local credit from spiking and memory claims. |
| 2 | publication_package | Report the supported mechanism chain through Gen-16 without architecture-superiority claims. | Exact protocols, seeds, checkpoints, causal controls, and negative gates are publication-ready. |
| 3 | gen17_sparse_spiking_translation | Translate the validated local rule to parameter-matched Bernoulli sensory events. | Sparse events preserve gain and reward identity with healthy activity under the frozen Gen-17 gate. |
| 4 | complex_plasticity_remains_gated | Keep STW/LTW, replay, and structural plasticity closed until sparse-spiking credit replicates. | No memory or topology mechanism is added before the sparse translation and replication gates pass. |
