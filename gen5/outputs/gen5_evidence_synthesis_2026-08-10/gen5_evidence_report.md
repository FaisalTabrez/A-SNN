...
| ---: | --- | --- | --- |
| 1 | publication_evidence_closeout | Package the supported event-audio mechanism and the Gen-19 event-vision boundary condition. | A reproducible 21-source ledger reports positive and negative gates without post-hoc rescue. |
| 2 | event_audio_mechanism_scope | Limit the causal sample-specific residual-state claim to SHD and SSC. | N-MNIST is presented as a failed external replication, not hidden by aggregate accuracy. |
| 3 | new_architecture_hypothesis | Require a separately theorized and preregistered mechanism before reopening event-vision state identity. | No parameter sweep or post-hoc Gen-19 rescue is labeled confirmatory evidence. |
| 4 | complex_plasticity_remains_gated | Keep local reward credit, STW/LTW, replay, and structural plasticity closed. | A future program must establish stable credit assignment before adding complex plasticity. |
