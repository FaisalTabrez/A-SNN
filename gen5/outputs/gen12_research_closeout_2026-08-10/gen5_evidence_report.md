...
| ---: | --- | --- | --- |
| 1 | gen12_terminal_closeout | Package conventional adaptation gains and the failed associative-prototype gate. | A reproducible 14-source ledger whose claims match the Gen-12 stop decision. |
| 2 | publication_package | Report the supported mechanism chain through Gen-12 without architecture-superiority claims. | Exact protocols, seeds, checkpoints, causal controls, and negative gates are publication-ready. |
| 3 | hardware_work_deferred | Keep synaptic STW/LTW, replay, structural plasticity, and event-driven kernel optimization closed after the Gen-12 terminal failure. | No hardware-efficiency claim is pursued for an architecture with no qualified causal arm. |
| 4 | local_credit_assignment_hypothesis | Test explicit three-factor local output-synapse updates rather than tune failed prototype retrieval. | Sparse local plasticity matches autograd readout adaptation and passes weight-removal and class-shuffle controls. |
