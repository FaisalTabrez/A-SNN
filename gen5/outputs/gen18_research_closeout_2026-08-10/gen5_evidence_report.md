...
| ---: | --- | --- | --- |
| 1 | gen18_local_credit_program_closeout | Package the exact gradient, positive mean, failed confidence bounds, and terminal stop. | A reproducible 20-source ledger closes local reward credit without post-hoc rescue. |
| 2 | publication_package | Report the supported mechanism chain through Gen-18 without reliable local-learning claims. | Exact protocols, seeds, checkpoints, causal controls, and negative gates are publication-ready. |
| 3 | gen19_nmnist_external_state_replication | Test causal residual LIF state on real event-vision data under matched accuracy and identity ablations. | N-MNIST reference, matched accuracy, state removal, shuffled identity, and activity gates all pass. |
| 4 | complex_plasticity_remains_gated | Keep local reward credit, STW/LTW, replay, and structural plasticity closed while external state evidence is tested. | Gen-19 changes dataset modality only; it does not reopen the failed learning rule. |
