...
| ---: | --- | --- | --- |
| 1 | gen13_terminal_closeout | Package conventional adaptation gains and the failed local-plasticity gate. | A reproducible 15-source ledger whose claims match the Gen-13 stop decision. |
| 2 | publication_package | Report the supported mechanism chain through Gen-13 without architecture-superiority claims. | Exact protocols, seeds, checkpoints, causal controls, and negative gates are publication-ready. |
| 3 | continual_adaptation_branch_closed | Keep STW/LTW, replay, structural plasticity, and local-rule tuning closed after the Gen-13 terminal failure. | No consolidation or autonomous-learning claim is made from a rule that missed adaptation and causal gates. |
| 4 | new_program_selection | Choose a genuinely new program: reward-modulated embodiment, external event-benchmark generalization, or publication closeout. | A separately preregistered question with a distinct mechanism and explicit stop rule is approved before more compute is spent. |
