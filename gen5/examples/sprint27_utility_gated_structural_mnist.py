"""Sprint 27 CLI: gradient-gated growth and peripheral pruning."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Phase 27 utility-gated structural plasticity"
    )
    ...
    parser.add_argument("--scoring-batches", type=int, default=4)
    parser.add_argument("--prune-after-epochs", type=int, default=3)
    ...
    parser.add_argument(
        "--output-dir", default="gen5_outputs/utility_gated_structural_mnist"
    )
...
def main() -> None:
    args = parse_args()
    if args.list_arms:
        print("\n".join(available_utility_gated_arms()))
        return
    ...
    result = run_utility_gated_structural_mnist(
        config,
        device=args.device,
        warmup_epochs=args.warmup_epochs,
        scoring_batches=args.scoring_batches,
    ...
    print(
        json.dumps(
            {"paths": paths, "device": result.device, "summary": result.summary},
            indent=2,
        )
    )
