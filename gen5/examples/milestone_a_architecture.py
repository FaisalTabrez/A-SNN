"""Milestone A CLI: screen, promote, and confirm SSC architectures."""
...
from ammc_gen5 import SHDConfig, available_milestone_a_arms, run_milestone_a  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the consolidated SSC architecture milestone: one-seed screening, "
            "validation-only promotion, three-seed confirmation, and causal ablation."
        )
    )
    ...
    parser.add_argument("--causal-margin", type=float, default=0.01)
    ...
    parser.add_argument(
        "--output-dir", default="gen5_outputs/milestone_a_architecture"
    )
    parser.add_argument(
        "--progress-path",
        default=None,
        help="checkpoint JSON; defaults to <output-dir>/milestone_a_progress.json",
    )
...
def main() -> None:
    args = parse_args()
    if args.list_arms:
        print("\n".join(available_milestone_a_arms()))
        return
    config = SHDConfig(
    ...
    progress_path = args.progress_path or str(
        pathlib.Path(args.output_dir) / "milestone_a_progress.json"
    )
    result = run_milestone_a(
        config,
