"""Sprint 40 CLI: decompose analog dynamics, leak, and sparse topology."""
...
from ammc_gen5 import SHDConfig, run_shd_analog_topology  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 40 SHD analog/topology controls")
    ...
    parser.add_argument("--dense-hidden-neurons", type=int, default=128)
    ...
    parser.add_argument("--sparse-projection-dim", type=int, default=32)
    parser.add_argument("--dense-projection-dim", type=int, default=16)
    ...
    parser.add_argument("--output-dir", default="gen5_outputs/shd_analog_topology")
...
def main() -> None:
    args = parse_args()
    config = SHDConfig(
        seeds=tuple(args.seeds), train_samples=args.train_samples, test_samples=args.test_samples,
        timesteps=args.timesteps, duration_seconds=args.duration_seconds,
        hidden_neurons=args.sparse_hidden_neurons, sensor_fanout=args.sensor_fanout,
        recurrent_fanout=args.recurrent_fanout, max_edges=args.max_edges,
        reservoir_leak=args.reservoir_leak, reservoir_threshold=args.reservoir_threshold,
        input_gain=args.input_gain, epochs=args.epochs, warmup_epochs=args.warmup_epochs,
        learning_rate=args.learning_rate, reservoir_learning_rate=args.reservoir_learning_rate,
        weight_decay=args.weight_decay, batch_size=args.batch_size, data_seed=args.data_seed,
        data_root=args.data_root, download=not args.no_download,
    )
    result = run_shd_analog_topology(
        config, sparse_hidden_neurons=args.sparse_hidden_neurons,
        dense_hidden_neurons=args.dense_hidden_neurons, device=args.device,
        surrogate_slope=args.surrogate_slope, sparse_projection_dim=args.sparse_projection_dim,
        dense_projection_dim=args.dense_projection_dim, temporal_levels=args.temporal_levels,
        readout_hidden_units=args.readout_hidden_units, ltw_minimum=args.ltw_minimum,
