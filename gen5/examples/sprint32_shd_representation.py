"""Sprint 32 CLI: diagnose the SHD sparse-representation gap."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Phase 32 SHD representation diagnostics"
    )
    ...
    parser.add_argument(
        "--output-dir", default="gen5_outputs/shd_representation"
    )
...
def main() -> None:
    args = parse_args()
    if args.list_arms:
        print("\n".join(available_shd_representation_arms()))
        return
    config = SHDConfig(
    ...
    result = run_shd_representation_diagnostic(
        config,
        device=args.device,
