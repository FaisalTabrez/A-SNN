"""Gen-6 successor CLI: shared zero-initialized state corrections on TCN."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the preregistered Gen-6 TCN plus weight-shared residual-state "
            "screen and automatic confirmation."
        )
    )
    ...
    parser.add_argument("--output-dir", default="gen5_outputs/gen6_successor")
...
def main() -> None:
    args = parse_args()
    if args.list_arms:
        print("\n".join(available_gen6_successor_arms()))
        return
    config = SHDConfig(
    ...
    progress_path = args.progress_path or str(
        pathlib.Path(args.output_dir) / "gen6_successor_progress.json"
    )
    result = run_gen6_successor(
        config,
