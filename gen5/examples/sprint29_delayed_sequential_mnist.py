"""Sprint 29 CLI: executable axonal delays on row-sequential MNIST."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the Phase 29 executable-delay sequential MNIST ablation"
    )
    ...
    parser.add_argument("--output-dir", default="gen5_outputs/delayed_sequential_mnist")
...
def main() -> None:
    args = parse_args()
    if args.list_arms:
        print("\n".join(available_delayed_sequential_arms()))
        return
    ...
    result = run_delayed_sequential_mnist(
        config,
        device=args.device,
