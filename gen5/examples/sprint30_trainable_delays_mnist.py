"""Sprint 30 CLI: optimize delay assignments on the fixed sparse graph."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Phase 30 trainable delay assignment on sequential MNIST"
    )
    ...
    parser.add_argument("--delay-learning-rate", type=float, default=0.003)
    parser.add_argument("--entropy-regularization", type=float, default=0.001)
    ...
    parser.add_argument("--output-dir", default="gen5_outputs/trainable_delays_mnist")
...
def main() -> None:
    args = parse_args()
    if args.list_arms:
        print("\n".join(available_trainable_delay_arms()))
        return
    ...
    result = run_trainable_delays_mnist(
        config,
        device=args.device,
        warmup_epochs=args.warmup_epochs,
        surrogate_slope=args.surrogate_slope,
        delay_learning_rate=args.delay_learning_rate,
