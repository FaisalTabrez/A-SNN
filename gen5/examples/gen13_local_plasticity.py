...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the preregistered Gen-13 local-plasticity experiment."
    )
    parser.add_argument("--list-strategies", action="store_true")
    parser.add_argument("--seeds", nargs="+", type=int, default=[160, 161, 162])
    ...
    parser.add_argument("--local-learning-rate", type=float, default=0.50)
    parser.add_argument("--local-weight-decay", type=float, default=0.0001)
    ...
    parser.add_argument("--output-dir", default="gen5_outputs/gen13_local_plasticity")
...
def main() -> None:
    ...
    if args.list_strategies:
        print("\n".join(available_gen13_plasticity_strategies()))
        return
    config = SHDConfig(
    ...
    progress_path = args.progress_path or str(
        pathlib.Path(args.output_dir) / "gen13_local_plasticity_progress.json"
    )
    result = run_gen13_local_plasticity(
