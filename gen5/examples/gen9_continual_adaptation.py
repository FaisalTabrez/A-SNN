"""Gen-9 controlled continual-adaptation CLI."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the preregistered Gen-9 sensor-damage continual-adaptation "
            "screen and automatic SSC confirmation."
        )
    )
    parser.add_argument("--list-arms", action="store_true")
    parser.add_argument("--screen-seed", type=int, default=148)
    parser.add_argument("--confirm-seeds", nargs="+", type=int, default=[148, 149, 150])
    ...
    parser.add_argument("--minimum-shift-drop", type=float, default=0.05)
    parser.add_argument("--minimum-adaptation-gain", type=float, default=0.02)
    parser.add_argument("--minimum-auc-advantage", type=float, default=0.01)
    ...
    parser.add_argument("--output-dir", default="gen5_outputs/gen9_continual_adaptation")
...
def main() -> None:
    ...
    if args.list_arms:
        print("Source arms:")
        print("\n".join(available_gen9_source_arms()))
        print("Adaptation strategies:")
        print("\n".join(available_gen9_adaptation_strategies()))
        return
    config = SHDConfig(
    ...
    progress_path = args.progress_path or str(
        pathlib.Path(args.output_dir) / "gen9_continual_adaptation_progress.json"
    )
    result = run_gen9_continual_adaptation(
        config,
        ...
        minimum_auc_advantage=args.minimum_auc_advantage,
        ...
        json.dumps(
            {
                ...
                "promoted_source_arms": result.promoted_source_arms,
                "decision": result.decision,
                "summary": result.adaptation_summary,
            },
            indent=2,
        )
    )
