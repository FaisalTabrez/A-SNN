"""Sprint 41 CLI: fixed-budget sparse leaky analog width scaling."""
...
from ammc_gen5 import SHDConfig, run_shd_sparse_width  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 41 SHD sparse width scaling")
    parser.add_argument("--seeds", nargs="+", type=int, default=[42, 43, 44])
    parser.add_argument("--widths", nargs="+", type=int, default=[128, 256, 512, 1024])
    ...
    parser.add_argument("--output-dir", default="gen5_outputs/shd_sparse_width")
...
def main() -> None:
    ...
    config = SHDConfig(
        ...
        hidden_neurons=max(args.widths), sensor_fanout=args.sensor_fanout,
        recurrent_fanout=args.recurrent_fanout, max_edges=args.max_edges,
        reservoir_leak=args.reservoir_leak, input_gain=args.input_gain,
    ...
    result = run_shd_sparse_width(
        config, widths=args.widths, target_parameters=args.target_parameters,
        device=args.device, surrogate_slope=args.surrogate_slope,
        projection_dim=args.projection_dim, temporal_levels=args.temporal_levels,
