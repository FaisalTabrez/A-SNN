"""Gen-8 time-local predictive-binding CLI."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the preregistered Gen-8 time-local predictive-binding screen "
            "and automatic SSC confirmation."
        )
    )
    parser.add_argument("--list-arms", action="store_true")
    parser.add_argument("--screen-seed", type=int, default=145)
    parser.add_argument("--confirm-seeds", nargs="+", type=int, default=[145, 146, 147])
    ...
    parser.add_argument("--binding-gain-margin", type=float, default=0.005)
    ...
    parser.add_argument("--output-dir", default="gen5_outputs/gen8_temporal_binding")
...
def main() -> None:
    args = parse_args()
    if args.list_arms:
        print("\n".join(available_gen8_temporal_binding_arms()))
        return
    config = SHDConfig(
    ...
    progress_path = args.progress_path or str(
        pathlib.Path(args.output_dir) / "gen8_temporal_binding_progress.json"
    )
    result = run_gen8_temporal_binding(
        config,
        ...
        binding_gain_margin=args.binding_gain_margin,
