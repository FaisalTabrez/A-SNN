"""Sprint 25 CLI: warm-started LTW learning on row-sequential MNIST."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the Phase 25 sequential LTW training benchmark"
    )
    ...
    parser.add_argument("--output-dir", default="gen5_outputs/trainable_sequential_mnist")
...
def main() -> None:
    args = parse_args()
    if args.list_arms:
        print("\n".join(available_sequential_ltw_arms()))
        return
    ...
    result = run_trainable_sequential_mnist(
        config,
        device=args.device,
