"""Sprint 28 CLI: adaptive-threshold ablation on row-sequential MNIST."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the Phase 28 adaptive-neuron sequential MNIST ablation"
    )
    ...
    parser.add_argument("--adaptation-decay", type=float, default=0.95)
    parser.add_argument("--adaptation-strength", type=float, default=0.5)
    ...
    parser.add_argument("--output-dir", default="gen5_outputs/adaptive_sequential_mnist")
...
def main() -> None:
    args = parse_args()
    if args.list_arms:
        print("\n".join(available_adaptive_sequential_arms()))
        return
    ...
    result = run_adaptive_sequential_mnist(
        config,
        device=args.device,
        warmup_epochs=args.warmup_epochs,
        surrogate_slope=args.surrogate_slope,
        adaptation_decay=args.adaptation_decay,
