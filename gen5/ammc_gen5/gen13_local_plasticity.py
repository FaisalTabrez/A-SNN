"""Gen-13 frozen sensory backbone with manual three-factor local plasticity."""
...
from .gen11_plastic_adapter import _adaptation_curve, _curve_auc, _train_dropout_backbone
from .gen12_associative_memory import _backbone_features, top_fraction_spike_code
from .gen9_continual_adaptation import apply_sensor_damage, sensor_damage_indices
from .milestone_a_architecture import _load_progress
...
def available_gen13_plasticity_strategies() -> tuple[str, ...]:
    return GEN13_PLASTICITY_STRATEGIES


def three_factor_update(
    fast_weights, presynaptic_trace, logits, labels, *, learning_rate: float,
    weight_decay: float = 0.0,
):
    """Apply a local pre × postsynaptic-error × modulator update in place."""

    probability = torch.softmax(logits, dim=1)
    target = torch.nn.functional.one_hot(
        labels, num_classes=fast_weights.shape[0]
    ).to(probability.dtype)
    postsynaptic_error = target - probability
    update = postsynaptic_error.transpose(0, 1) @ presynaptic_trace
    update = update / max(int(labels.shape[0]), 1)
    if weight_decay:
        fast_weights.mul_(1.0 - learning_rate * weight_decay)
    fast_weights.add_(learning_rate * update)
    return postsynaptic_error


@dataclass
class Gen13LocalPlasticityResult:
    config: SHDConfig
    device: str
    ...
    def save(self, output_dir: str | pathlib.Path, *, plot: bool = True) -> dict[str, str]:
        ...
        json_path = output / "gen13_local_plasticity.json"
        records_path = output / "gen13_local_plasticity_records.csv"
        summary_path = output / "gen13_local_plasticity_summary.csv"
        payload = {
        ...
        if plot and self.summary:
            plot_path = output / "gen13_local_plasticity.png"
            plot_gen13_local_plasticity(self.records, self.summary, plot_path)
            paths["plot"] = str(plot_path)
        return paths


def run_gen13_local_plasticity(
    config: SHDConfig,
    *,
    seeds: Iterable[int] = (160, 161, 162),
    ...
    local_learning_rate: float = 0.50,
    local_weight_decay: float = 0.0001,
    ...
    if torch is None:
        raise ImportError("Gen-13 local plasticity requires PyTorch")
    ...
    _validate_run(
        ...
        adaptation_learning_rate, local_learning_rate, local_weight_decay,
        spike_fraction,
    ...
    signature = _run_signature(
        ...
        local_weight_decay=local_weight_decay, spike_fraction=spike_fraction,
    ...
    if progress.get("stage") == "complete":
        return _completed_result(
            progress, full_config, resolved, target_parameters, levels,
            seed_values, source_mask_fraction, damage_fraction, damage_seed,
            budgets, local_learning_rate, spike_fraction,
            minimum_shift_drop, minimum_adaptation_gain, auc_margin,
            final_accuracy_margin, forgetting_margin, causal_margin,
            minimum_spike_density, maximum_spike_density,
        )
    ...
    summary = summarize_gen13_local_plasticity(records, budgets=budgets)
    ...
    return _result(
        ...
        local_learning_rate, spike_fraction, records, summary, decision,
    )


def summarize_gen13_local_plasticity(
    ...
    for strategy in GEN13_PLASTICITY_STRATEGIES:
        ...
        for seed in sorted({int(row["seed"]) for row in group}):
            ...
            per_seed.append({
                ...
                "fast_contribution": float(final.get("fast_weight_contribution") or 0.0),
                "class_specificity": float(final.get("class_specificity") or 0.0),
                "throughput": float(final["test_examples_per_second"]),
                "seconds": float(final["cumulative_adaptation_seconds"]),
                "active": int(final.get("active_fast_synapses") or 0),
                "magnitude": float(final.get("mean_absolute_fast_weight") or 0.0),
            })
        summary.append({
            "strategy": strategy,
            "runs": len(per_seed),
            ...
            "mean_fast_weight_contribution": statistics.fmean(item["fast_contribution"] for item in per_seed),
            "fast_weight_contribution_seed_count": sum(item["fast_contribution"] >= 0.005 for item in per_seed),
            "mean_class_specificity": statistics.fmean(item["class_specificity"] for item in per_seed),
            "class_specificity_seed_count": sum(item["class_specificity"] >= 0.005 for item in per_seed),
            ...
            "mean_active_fast_synapses": statistics.fmean(item["active"] for item in per_seed),
            "mean_absolute_fast_weight": statistics.fmean(item["magnitude"] for item in per_seed),
...
def decide_gen13_local_plasticity(
    ...
    if any(name not in lookup for name in names):
        return {
            ...
            "next_milestone": "close_gen13_local_plasticity",
        }
    static, readout, local = (lookup[name] for name in names)
    required = 2 if int(local["runs"]) >= 3 else 1
    passed = (
        float(static["mean_shift_drop"]) >= minimum_shift_drop
        and float(local["mean_adaptation_gain"]) >= minimum_adaptation_gain
        and int(local["two_point_gain_seed_count"]) >= required
        and float(local["mean_adaptation_auc"]) >= float(readout["mean_adaptation_auc"]) - auc_margin
        and float(local["mean_shifted_final_accuracy"]) >= float(readout["mean_shifted_final_accuracy"]) - final_accuracy_margin
        and float(local["mean_forgetting"]) <= float(readout["mean_forgetting"]) + forgetting_margin
        and float(local["mean_fast_weight_contribution"]) >= causal_margin
        and int(local["fast_weight_contribution_seed_count"]) >= required
        and float(local["mean_class_specificity"]) >= causal_margin
        and int(local["class_specificity_seed_count"]) >= required
        and minimum_spike_density <= float(local["mean_activity"]) <= maximum_spike_density
    )
    return {
        "status": "pass" if passed else "stop",
        "qualified_arms": ["spiking_three_factor_readout"] if passed else [],
        "next_milestone": (
            "stw_ltw_consolidation_preregistration"
            if passed else "close_gen13_local_plasticity"
        ),
    }


def plot_gen13_local_plasticity(records, summary, path) -> None:
    ...
    axes[1, 0].bar(labels, [100.0 * row["mean_fast_weight_contribution"] for row in summary], color="#167d55")
    axes[1, 0].axhline(0.5, color="#bd3d3a", linestyle="--")
    axes[1, 0].set_ylabel("Full - fast weights removed (points)")
    axes[1, 1].bar(labels, [100.0 * row["mean_class_specificity"] for row in summary], color="#35b4f2")
    axes[1, 1].axhline(0.5, color="#bd3d3a", linestyle="--")
    axes[1, 1].set_ylabel("Full - shuffled output classes (points)")
...
def _run_all_strategies(
    ...
    adaptation_epochs_per_block, adaptation_learning_rate,
    local_learning_rate, local_weight_decay, spike_fraction,
    target_parameters, levels, input_kernel_size, hidden_kernel_size,
    tcn_dilation, device, existing_records=(), progress_callback=None,
):
    ...
    for seed in seeds:
        expected = {
            ...
            for strategy in GEN13_PLASTICITY_STRATEGIES for budget in budgets
        }
        ...
        for strategy in GEN13_PLASTICITY_STRATEGIES:
            ...
            if strategy in {
                ...
                for row in new_rows:
                    row.update({
                        "fast_weight_contribution": None,
                        "class_specificity": None,
                        "active_fast_synapses": 0,
                        "mean_absolute_fast_weight": 0.0,
                    })
                del model
            else:
                new_rows = _local_curve(
                    backbone, strategy, seed,
                    validation_events, validation_labels,
                    test_events, test_labels, fixed_mask=fixed_mask,
                    budgets=budgets, levels=levels,
...
def _local_curve(
    ...
    epochs_per_block, local_learning_rate, local_weight_decay,
    spike_fraction, batch_size, device,
):
    spiking = strategy == "spiking_three_factor_readout"
    feature_dim = backbone.channels * sum(levels)
    fast_weights = torch.zeros(
        (backbone.config.classes, feature_dim), device=device
    )
    generator = torch.Generator(device="cpu").manual_seed(seed + 114_000)
    sample_order = torch.randperm(adaptation_events.shape[0], generator=generator)
    ...
    for budget in budgets:
        if budget > previous:
            indices = sample_order[previous:budget]
            sync(device)
            started = time.perf_counter()
            for _ in range(epochs_per_block):
                epoch_order = indices.index_select(
                    0, torch.randperm(indices.shape[0], generator=generator)
                )
                for offset in range(0, epoch_order.shape[0], batch_size):
                    index = epoch_order[offset: offset + batch_size]
                    ...
                    with torch.no_grad():
                        features, base_logits = _backbone_features(backbone, events, levels)
                        trace = top_fraction_spike_code(features, spike_fraction) if spiking else features
                        trace = torch.nn.functional.normalize(trace, dim=1)
                        logits = base_logits + trace @ fast_weights.transpose(0, 1)
                        three_factor_update(
                            fast_weights, trace, logits, labels,
        ...
        full_accuracy, elapsed, activity = _measure_local(
            backbone, test_events, test_labels, fixed_mask=fixed_mask,
            fast_weights=fast_weights, levels=levels,
            spike_fraction=spike_fraction, spiking=spiking,
            batch_size=batch_size, device=device, mode="full",
        )
        removed_accuracy = _measure_local(
            backbone, test_events, test_labels, fixed_mask=fixed_mask,
            fast_weights=fast_weights, levels=levels,
            spike_fraction=spike_fraction, spiking=spiking,
            batch_size=batch_size, device=device, mode="removed",
        )[0]
        shuffled_accuracy = _measure_local(
            backbone, test_events, test_labels, fixed_mask=fixed_mask,
            fast_weights=fast_weights.roll(shifts=1, dims=0), levels=levels,
            spike_fraction=spike_fraction, spiking=spiking,
            batch_size=batch_size, device=device, mode="full",
        )[0]
        rows.append({
            ...
            "shifted_accuracy": full_accuracy,
            "activity": activity,
            "state_contribution": None,
            "state_specificity": None,
            "mean_absolute_gate": 0.0,
            "fast_weight_contribution": full_accuracy - removed_accuracy,
            "class_specificity": full_accuracy - shuffled_accuracy,
            "active_fast_synapses": int((fast_weights.abs() > 1e-6).sum().item()),
            "mean_absolute_fast_weight": float(fast_weights.abs().mean().item()),
...
def _measure_local(
    backbone, events, labels, *, fixed_mask, fast_weights, levels,
    spike_fraction, spiking, batch_size, device, mode,
):
    backbone.eval()
    correct = total = 0
    activity_sum = 0.0
    ...
    with torch.no_grad():
        for offset in range(0, events.shape[0], batch_size):
            ...
            trace = top_fraction_spike_code(features, spike_fraction) if spiking else features
            activity_sum += float((trace != 0).to(torch.float32).mean().item()) * int(batch_labels.shape[0])
            trace = torch.nn.functional.normalize(trace, dim=1)
            logits = base_logits
            if mode == "full":
                logits = logits + trace @ fast_weights.transpose(0, 1)
...
def _completed_result(
    progress, config, device, target_parameters, levels, seeds,
    source_mask_fraction, damage_fraction, damage_seed, budgets,
    local_learning_rate, spike_fraction, minimum_shift_drop,
    minimum_adaptation_gain, auc_margin, final_accuracy_margin,
    forgetting_margin, causal_margin, minimum_spike_density,
    maximum_spike_density,
):
    records = list(progress.get("records", []))
    summary = summarize_gen13_local_plasticity(records, budgets=budgets)
    decision = progress.get("decision") or decide_gen13_local_plasticity(
        summary, minimum_shift_drop=minimum_shift_drop,
        minimum_adaptation_gain=minimum_adaptation_gain, auc_margin=auc_margin,
        final_accuracy_margin=final_accuracy_margin,
        forgetting_margin=forgetting_margin, causal_margin=causal_margin,
    ...
    return _result(
        ...
        damage_fraction, damage_seed, budgets, local_learning_rate,
...
def _result(
    ...
    return Gen13LocalPlasticityResult(
        ...
        adaptation_budgets=budgets, local_learning_rate=local_learning_rate,
        spike_fraction=spike_fraction, records=records,
...
def _validate_run(
    ...
    damage_fraction, adaptation_epochs, adaptation_lr, local_lr,
    local_decay, spike_fraction, gates, spike_gate,
):
    ...
    if budgets[-1] > 9981 or adaptation_lr <= 0.0 or local_lr <= 0.0:
        raise ValueError("invalid adaptation budget or learning rate")
    if local_decay < 0.0:
        raise ValueError("local weight decay must be non-negative")
...
def _run_signature(config, **values):
    signature = {
        "version": 1,
        "experiment": "gen13_local_plasticity",
        "strategies": list(GEN13_PLASTICITY_STRATEGIES),
